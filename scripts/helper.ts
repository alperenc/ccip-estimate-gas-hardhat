 ...
 * @param param1 An object containing optional senderAddress, swapperAddress and receiverAddress to be saved.
 */
export async function createOrUpdateConfigFile(
  network: SupportedNetworks,
  {
    ...
    receiverAddress,
    swapperAddress,
  }: { senderAddress?: string; receiverAddress?: string; swapperAddress?: string }
) {
  ...
  if (fs.existsSync(deployedContracts)) {
    ...
    // Update sender, swapper and receiver addresses if provided.
    if (senderAddress) config[network].sender = senderAddress;
    if (swapperAddress) config[network].swapper = swapperAddress;
