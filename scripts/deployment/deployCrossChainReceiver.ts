...
async function deployAndVerifyReceiver(network: SupportedNetworks) {
    ...
    const { router, comet } = getCCIPConfig(network);

    const swapperAddress = (
        deployedContracts[network] as { swapper: string }
    ).swapper;
    ...
    const receiver = await Receiver.deploy(router, comet, swapperAddress);
    ...
    if (tx) {
        ...
        try {
            ...
            await run("verify:verify", {
                ...
                constructorArguments: [router, comet, swapperAddress],
            });
